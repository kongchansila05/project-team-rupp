@extends('layouts.backend.app',[
    ...
    'pageTitle' => 'Edit Purchases',
])
@section('purchases-active', 'active')
@section('edit_purchases', 'active')
...
<script type="text/javascript">
    ...
    var sale_item = {!! json_encode($item->toArray()) !!}
    if(sale_item){
        $.each(sale_item,function(index,row){
            add_item = JSON.parse(localStorage.getItem('product'));
            langdata = add_item ? add_item.length : '';
                if(langdata == 0){
                    add_item = {};
                }
                add_item[new Date().getTime()] = row;
...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
    <!-- Page Heading -->
    <form action="/purchases/update" method="POST" enctype="multipart/form-data">
        @csrf
    <div class="row">
        <div class="col-md-4">
            <div class="form-group">
                <label for="reference">Reference No</label>
                <input type="text" class="form-control" id="reference" name="reference" value="{{$purchases[0]->reference_no}}">
                @error('reference')
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="date" name="date" value="{{$purchases[0]->date}}">
            </div>
        </div>
        <input hidden type="text" class="form-control" id="p_id" name="p_id" value="{{$id}}">
    </div>
    <div class="row">
        <div class="col-md-12" id="sticker" style="width: 1027px; z-index: 1;">
    ...
    <div class="row">
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="total_discount" name="total_discount" value="{{$purchases[0]->total_discount}}">
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="shipping" name="shipping" value="{{$purchases[0]->shipping}}">
                @error('shipping')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
    ...
    <div class="form-group">
        ...
        height: 125px;" name="note">{{$purchases[0]->note}}</textarea>
        @error('note')
            <small class="text-danger">{{ $message }}</small>
        @enderror
    </div>
    <button type="submit" class="btn btn-primary">Edit Purchases</button>
