...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        select: function (event, ui) {
                if (ui.item.id !== 0) {
                    var row = add_process_list(ui.item);
                    if (row)
                    $(this).val('');
                } else {
                    bootbox.alert('no_match_found');
    ...
    function add_process_list(item) {
        add_item = JSON.parse(localStorage.getItem('process'));
        langdata = add_item ? add_item.length : '';
            if(langdata == 0){
                add_item = {};
            }
            add_item[new Date().getTime()] = item;
            localStorage.setItem('process', JSON.stringify(add_item));
            loadProcess();
        return true;
    }
    ...
    $(document).on('click', '.delete', function () {
            var id = $(this).parent().find(".pid").val();
            var storage = JSON.parse(localStorage.getItem('process'));
            var newstorage=[];
            $.each(storage,function(index,row){
                if(index == id){
                }else{
        ...
        localStorage.setItem('process', JSON.stringify(newstorage));
        loadProcess()
    });
    function loadProcess() {
        $('#prTable tbody').empty();
            ...
            $('#prTable tbody').append("<tr>"+
                "<td>"+'<input name="process_row[]" type="hidden" class="pid" value="' + index +'">'
                +row.name+" ( "+row.code+" ) </td>"
                +'<td class="rec_con"><input class="form-control text-center process_quantity" name="process_quantity[]" type="text" value="' +row.quantity+'" onClick="this.select();"></td>'
                +'<td class="delete">'+'<i class="fas fa-trash"></i>'+"</td>"
                +"</tr>"
                +'<input name="process_id[]" type="hidden" class="process_id" value="' + row.value +'">'
                +'<input name="process_code[]" type="hidden" class="process_code" value="' + row.code +'">'
                +'<input name="process_type[]" type="hidden" class="process_type" value="' + row.type +'">'
                +'<input name="process_name[]" type="hidden" class="process_name" value="' + row.name +'">'
                )
            }) 
    };
    ...
    $(document).on('change', '.process_quantity', function () {
        ...
        localStorage.setItem('process', JSON.stringify(storage));
        loadProcess()
    });
</script>
@endpush
...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
    <!-- Page Heading -->
    <form action="/process/store" method="POST" enctype="multipart/form-data">
        @csrf
    <div class="row">
        <div class="col-md-4">
    ...
    <div class="col-md-12 row">
            ...
            <div class="well well-sm">
                <div class="form-group " style="margin-bottom:8px;">
                    <div class="input-group wide-tip">
                        <div class="input-group-addon" style="padding-left: 10px; padding-right: 10px;">
                            <i class="fa fa-2x fa-barcode addIcon"></i></div>
                            <input class="form-control" type="text" id='process_search' placeholder="Please add middle to order list">
                        <div class="input-group-addon" style="padding-left: 10px; padding-right: 10px;">
                        <a href="/product/create" id="addManually1" tabindex="-1"><i class=" fa fa-2x fa-plus-circle addIcon" style="color: #36b9cc"></i></a></div>
                    </div>
                </div>
        ...
        <div class="col-md-12 row">
            ...
            <div class="col-md-6">
                <div class="control-group table-group">
                    <label class="table-label">Middle</label>
                    <div class="controls table-controls">
                    <table id="prTable" class=" bg-gradient-info table items table-striped table-bordered">
                        <thead>
                        <tr class="text-center" style="color: white" >
                            <th class="col-md-4">Product (Code - Name)</th>
                            <th style="width: 10%">Quantity</th>
                            <th style="width: 1% !important;" id="remove_process"><i class="fas fa-trash"></i></th>
                        </tr>
                        </thead>
                        <tbody></tbody>
                        <tfoot></tfoot>
    ...
    <a href="/process" class="btn btn-secondary ">Cancel</a>
    </form>
    </div>
</div>
@endsection
